<template>
    <div class="edit-tenant container">
        ...
        <form @submit.prevent="editTenant" v-if="tenant">
            ...
            <router-link :to="{name: 'Tenants'}" class="left btn yellow darken-4" value="Cancel">Cancel</router-link>
...
export default {
  ...
  created() {
    ...
    this.tenantRef.get().then(doc => {
      ...
      db.collection("plots")
        .where("tenant", "==", this.$route.params.tenant_id)
        .get()
        .then(plots => {
          plots.forEach(plot => {
            let thisPlot = {
    ...
    this.plotRef
      ...
      .then(plots => {
        plots.forEach(plot => {
          this.availablePlots.push({ id: plot.id, size: plot.data().size });
        });
      });
  },
  methods: {
    editTenant() {
      if (this.tenant.lastName) {
        this.tenantRef
          ...
          .then(doc => {
            this.availablePlots.filter(
              plot =>
                plot.changed
                  ? db.doc(`plots/${plot.id}`).update({ tenant: "nobody" })
                  : null
            );
            this.tenantPlots.forEach(plot =>
              db.doc(`plots/${plot.id}`).update({ tenant: this.tenant.id })
            );
