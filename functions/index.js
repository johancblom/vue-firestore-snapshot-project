...
exports.addedTenant = functions.firestore
  ...
  .onCreate((snap, context) => {
    const myPdfFile = admin
      .storage()
      .bucket()
      .file("/tenants/" + snap.id + ".pdf");
...
exports.changedTenant = functions.firestore
  ...
  .onUpdate((change, context) => {
    ...
    const allTenantsFile = admin
      .storage()
      .bucket()
      .file("/tenants/allTenants.pdf");
    ...
    return admin
      ...
      .then(snap => {
        let result = snap.docs
          .filter(tenant => tenant.id != "nobody")
          .map(tenant => {
            return new Promise(resolve =>
              createInvoice(tenant, tenantsDoc, resolve)
            );
          });
